...
def pd_serie_to_distribution(serie : pd.Series, n_value_max : int) -> np.array:
    ...
    assert 0 <= serie.min() and serie.max() <= n_value_max, f"Values should be in the range [0, {n_value_max}], found {serie.min()} and {serie.max()} instead."
    res = serie.value_counts(sort = True, normalize = True, ascending = False, bins = range(n_value_max + 2))
    vector_distrib = res.values / np.sum(res.values)
    assert vector_distrib.shape == (n_value_max + 1,), f"Expected shape {(n_value_max + 1,)}, got {vector_distrib.shape} instead."
...
loss_name_to_loss_fn : Dict[str, Callable[[np.array, np.array], float]] = {
    ...
    'sum_ratio_abs_diff' : lambda x, y: np.sum(np.abs(x - y) / (x + sys.float_info.epsilon)) / len(x),
    }
...
def compute_distribution_difference(
    ...
    vector_distribution1 = pd_serie_to_distribution(feature_serie1, n_value_max = n_value_max)
    vector_distribution2 = pd_serie_to_distribution(feature_serie2, n_value_max = n_value_max)
    ...
    for loss_name, loss_fn in loss_name_to_loss_fn.items():
        ...
        if normalization_method == "typical_shift":
            ...
            for i in range(n_monte_carlo):
                ...
                estimated_typical_loss_value += (loss_fn(vector_distribution1, vector_distribution1_noisy) + loss_fn(vector_distribution2, vector_distribution2_noisy)) / 2
            ...
            estimated_typical_loss_value = 0
            for i in range(n_monte_carlo):
                vector_distribution1_sampled_a = sample_empirical_distribution(p = vector_distribution1, n = n1)
                vector_distribution1_sampled_b = sample_empirical_distribution(p = vector_distribution1, n = n1)
                vector_distribution2_sampled_a = sample_empirical_distribution(p = vector_distribution2, n = n2)
                vector_distribution2_sampled_b = sample_empirical_distribution(p = vector_distribution2, n = n2)
                estimated_typical_loss_value += (loss_fn(vector_distribution1_sampled_a, vector_distribution1_sampled_b) + loss_fn(vector_distribution2_sampled_a, vector_distribution2_sampled_b)) / 2
            estimated_typical_loss_value /= n_monte_carlo
        
        else:
            raise ValueError(f"normalization_method should be either 'typical_shift' or 'mc_estimated_loss', found {normalization_method} instead.")
        
        loss_value_normalized = loss_value / (estimated_typical_loss_value + sys.float_info.epsilon)
        loss_name_to_loss_values[loss_name] = {
            "loss_value" : loss_value,
