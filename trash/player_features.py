...
import numpy as np
import pandas as pd
from features_creators.base_feature_creator import BaseFeatureCreator
...
class PlayerFeaturesCreator(BaseFeatureCreator):
    """The class for creating the player features."""

    def create_usable_features(self, feature_dict_intermediary_objects : Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        """
        self.dataframe_playerfeatures = feature_dict_intermediary_objects["playerfeatures"]

        # Create smart features
        offensive_positive_playerfeature = self.extract_sum(OFFENSIVE_POSITIVE_PLAYERFEATURES)
        offensive_negative_playerfeature = self.extract_sum(OFFENSIVE_NEGATIVE_PLAYERFEATURES)
        defensive_positive_playerfeature = self.extract_sum(DEFENSIVE_POSITIVE_PLAYERFEATURES)
        defensive_negative_playerfeature = self.extract_sum(DEFENSIVE_NEGATIVE_PLAYERFEATURES)
        shooting_positive_playerfeature = self.extract_sum(SHOOTING_POSITIVE_PLAYERFEATURES)
        shooting_negative_playerfeature = self.extract_sum(SHOOTING_NEGATIVE_PLAYERFEATURES)
        footwork_positive_playerfeature = self.extract_sum(FOOTWORK_POSITIVE_PLAYERFEATURES)
        footwork_negative_playerfeature = self.extract_sum(FOOTWORK_NEGATIVE_PLAYERFEATURES)
        tiredness_playerfeature = self.extract_sum(TIREDNESS_PLAYERFEATURES)
        goalkeeper_playerfeature = self.extract_sum(GOALKEEPER_PLAYERFEATURES)
        behavior_positive_playerfeature = self.extract_sum(BEHAVIOR_POSITIVE_PLAYERFEATURES)
        behavior_negative_playerfeature = self.extract_sum(BEHAVIOR_NEGATIVE_PLAYERFEATURES)
        misc_playerfeature = self.extract_sum(MISC_PLAYERFEATURES)
