...
```{r}
plot(fit_tgcca, display_order = FALSE, comp=1)
```
...
```{r}
colnames(player_home[,-c(1)])[which(abs(fit_tgcca$factors[[2]][[1]][,1]) > 0.2)]
colnames(player_away[,-c(1)])[which(abs(fit_tgcca$factors[[4]][[1]][,1]) > 0.2)]

rownames(player_home[,-c(1)])[which(abs(fit_tgcca$factors[[2]][[2]][,1]) > 0.2)]
...
```{r}
plot(fit_rgcca, display_order = FALSE, comp=1)
```
